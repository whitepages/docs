      ...
      "person_owners": [
        {
          ...
          "current_addresses": [...],
          "phones": [
            { "number": "(206) 555-0100", "type": "landline", "score": 70 }
          ],
...
| `owners[].emails: ["a@..."]`    | `result.ownership_info.person_owners[].emails: [{ "email": "a@...", "score": 88 }]` | Emails on owner records are now objects carrying a confidence score.                                                                                                         |
| `owners[].phones[].score`       | `result.ownership_info.person_owners[].phones[].score`                              | Retained. Owner and resident phones carry a confidence score, and phone `type` and `number` use the same vocabulary and format as the person endpoints.                      |
...
| `result_metadata` on responses | _(not present)_                             | Owners carry `person_owner_metadata` and residents carry `resident_metadata`, reporting display limits and counts of records held back. The response also carries a top-level `property_metadata` describing address match quality. |
