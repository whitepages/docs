...
<Callout type="info" title="Supported Regions">
  Only Texas and New York and their counties are supported. Use `state:tx` or
  `county:tx.{county}` (e.g., `county:tx.travis`) for Texas, or `state:ny` or
  `county:ny.{county}` (e.g., `county:ny.new_york`) for New York. To find valid
  region values, see the [Regions API](/references/regions).
</Callout>
