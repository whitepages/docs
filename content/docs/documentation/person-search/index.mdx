...
<Callout type="info" title="result_metadata">
  ...
  The full lists will always remain available at `details_url` (`/v2/person/{id}
  `); `additional` is the forward-compatible signal for how many more entries
